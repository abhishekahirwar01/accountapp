...
// Dynamic step fields based on user role
...
export function CompanyForm({ company, clients = HARDCODED_DATA.clients, onFormSubmit, onCancel }) {
  ...
  // Check user role - hardcoded for demo, you can get this from your auth context
  const [userRole] = useState('admin'); // 'admin' or 'customer'
  ...
  const validateStep = (stepNumber) => {
    const fields = getStepFields(isClient)[stepNumber];
    ...
    for (const field of fields) {
      ...
      if (field === 'client' && !formData[field] && !isClient) {
      ...
      if (field === 'businessName' && !formData[field]) {
        Alert.alert('Validation Error', 'Business name is required');
        return false;
      }
      
      if (field === 'registrationNumber' && !formData[field]) {
  ...
  const handleSubmit = async () => {
    ...
    setTimeout(() => {
      ...
      const submissionData = {
        ...
        ...(formData.client && !isClient && {
          clientInfo: clients.find(c => c._id === formData.client)
        })
      };
    ...
    if (step === 1) {
      return (
        <View style={styles.stepContainer}>
          {/* Client selection - only show for admin */}
          {!isClient && renderInputField('client')}
          ...
          <View style={styles.formItem}>
            ...
            <TouchableOpacity 
              ...
              onPress={() => {
                // Simulate file picker - in real app, use react-native-image-picker
                Alert.alert('Info', 'In real app, this would open image picker');
              }}
            >
      ...
      <View style={styles.container}>
        ...
        <View style={styles.stepper}>
          {stepLabels.map(({ number, label }) => (
            <View key={number} style={styles.stepperItem}>
              <TouchableOpacity
                style={[
                  styles.stepCircle,
                  step === number && styles.stepCircleActive,
                  step > number && styles.stepCircleCompleted,
                ]}
                onPress={() => {
                  if (number < step) {
              ...
              >
                {step > number ? (
                  ...
                  <Text style={[
                    styles.stepText,
                    step === number && styles.stepTextActive,
                    step > number && styles.stepTextCompleted,
                  ]}>
                    {number}
                  </Text>
                )}
              ...
              <Text style={[
                styles.stepLabel,
                step === number && styles.stepLabelActive,
              ]}>
                {label}
              </Text>
            </View>
        ...
        {renderModal(
          ...
          HARDCODED_DATA.businessTypes.map(type => ({ label: type, value: type })),
