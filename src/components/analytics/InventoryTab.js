...
const idOf = (v) => typeof v === "string" ? v : v?._id || v?.id || v?.$oid || "";
...
export default function InventoryTab({
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((p) => p.company && idOf(p.company) === selectedCompanyId);
  }, [products, selectedCompanyId]);

  const filteredServices = useMemo(() => {
    if (!selectedCompanyId) return services;
    return services.filter((s) => s.company && idOf(s.company) === selectedCompanyId);
  ...
  const fetchInventory = useCallback(async () => {
    ...
    try {
      ...
      const [pRes, sRes] = await Promise.all([
        fetch(`${BASE_URL}/api/products?clientId=${selectedClient._id}`, { headers }),
        fetch(`${BASE_URL}/api/services?clientId=${selectedClient._id}`, { headers })
      ]);
  ...
  const renderProductItem = ({ item }) => {
    ...
    // Dynamic color logic
    const stockStyle = stockCount > 10 ? styles.stockHigh : stockCount > 0 ? styles.stockLow : styles.stockOut;
    const stockTextStyle = stockCount > 10 ? styles.textHigh : stockCount > 0 ? styles.textLow : styles.textOut;
      ...
      <Surface style={styles.proCard} elevation={1}>
        <View style={styles.cardHeader}>
          ...
          <View style={styles.titleArea}>
            ...
            <Text style={styles.itemSub}>{item.company?.businessName || "No Company"}</Text>
          </View>
          <View style={[styles.stockBadge, stockStyle]}>
             <Text style={[styles.stockQtyText, stockTextStyle]}>{stockCount}</Text>
             <Text style={[styles.stockUnitText, stockTextStyle]}>{item.unit || 'pcs'}</Text>
          </View>
        </View>
        ...
        <View style={styles.statsGrid}>
          <View style={styles.statItem}>
            ...
            <Text style={styles.statValue}>{formatCurrencyINR(item.sellingPrice)}</Text>
          </View>
          <View style={styles.statItem}>
            <Text style={styles.statLabel}>COST PRICE</Text>
            <Text style={styles.statValueMuted}>{formatCurrencyINR(item.costPrice)}</Text>
          </View>
        </View>
    ...
    <Surface style={styles.proCard} elevation={1}>
      <View style={styles.cardHeader}>
        ...
        <View style={styles.titleArea}>
          ...
          <Text style={[styles.itemSub, { color: '#8b5cf6' }]}>Service Entry</Text>
        </View>
      </View>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabBar}>
        <TouchableOpacity onPress={() => setActiveTab('products')} style={[styles.tab, activeTab === 'products' && styles.activeTab]}>
          <Text style={[styles.tabText, activeTab === 'products' && styles.activeTabText]}>Products ({filteredProducts.length})</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => setActiveTab('services')} style={[styles.tab, activeTab === 'services' && styles.activeTab]}>
          <Text style={[styles.tabText, activeTab === 'services' && styles.activeTabText]}>Services ({filteredServices.length})</Text>
        </TouchableOpacity>
      </View>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={activeTab === 'products' ? renderProductItem : renderServiceItem}
...
const styles = StyleSheet.create({
  ...
  proCard: { backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 12, borderWeight: 1, borderColor: '#f1f5f9' },
  ...
  stockBadge: { alignItems: 'center', justifyContent: 'center', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, minWidth: 50 },
  ...
  statsGrid: { flexDirection: 'row', borderTopWidth: 1, borderTopColor: '#f8fafc', paddingTop: 12 },
  statItem: { flex: 1 },
  statLabel: { fontSize: 9, fontWeight: '800', color: '#cbd5e1', letterSpacing: 0.5 },
  statValue: { fontSize: 15, fontWeight: '700', color: '#1e293b' },
  statValueMuted: { fontSize: 14, fontWeight: '600', color: '#64748b' },

  cardFooter: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 12, paddingTop: 8, borderTopWidth: 1, borderTopColor: '#f8fafc' },
